...
void main() {
  ...
  test('VaultService initializes default Personal vault when registry is empty', () async {
    ...
    final globalSysDir = await container.read(storagePathServiceProvider).getGlobalRegistryDirectory();
  ...
  test('VaultService migrates global databases to Personal vault on first launch', () async {
    ...
    final oldBaishouWal = File(p.join(globalSysDir.path, 'baishou.sqlite-wal'));
    ...
    final newBaishouWal = File(p.join(personalSysDir.path, 'baishou.sqlite-wal'));
  ...
  test('VaultService can switch to a new workspace and create directories', () async {
    final vaultService = container.read(vaultServiceProvider.notifier);
    ...
    final workJournalsDir = await pathService.getJournalsBaseDirectory('Work');
  ...
  test('VaultService updates lastAccessedAt when switching to existing workspace', () async {
    final vaultService = container.read(vaultServiceProvider.notifier);
    await container.read(vaultServiceProvider.future); // Init Personal

    await vaultService.switchVault('Work'); // Create Work
    
    final personalVaultBefore = vaultService.getAllVaults().firstWhere((v) => v.name == 'Personal');
    final workVaultBefore = vaultService.getAllVaults().firstWhere((v) => v.name == 'Work');
    ...
    final personalVaultAfter = vaultService.getAllVaults().firstWhere((v) => v.name == 'Personal');
    final workVaultAfter = vaultService.getAllVaults().firstWhere((v) => v.name == 'Work');

    expect(personalVaultAfter.lastAccessedAt.isAfter(personalVaultBefore.lastAccessedAt), isTrue);
    expect(workVaultAfter.lastAccessedAt, equals(workVaultBefore.lastAccessedAt));
  });
  ...
  test('VaultService handles partial migration when only some legacy databases exist', () async {
    final pathService = container.read(storagePathServiceProvider);
    final globalSysDir = await pathService.getGlobalRegistryDirectory();
  ...
  test('VaultService vector migration edge case: intact sqlite-vec and fts5 data after physical move', () async {
    final pathService = container.read(storagePathServiceProvider);
    final globalSysDir = await pathService.getGlobalRegistryDirectory();
    ...
    expect(stats['total_count'], 1, reason: 'Vector metadata should survive migration');
    ...
    expect(vectorResults.length, 1, reason: 'sqlite-vec KNN search should work after physical file move');
    expect(vectorResults.first['message_id'], 'msg_1');
    
    // FTS full text search
    final newFtsResults = await newAgentDb.searchFts('Migration');
    expect(newFtsResults.length, 1, reason: 'FTS5 search index should survive physical file move');
