...
void main() {
  group('AgentDatabase — Cross-Dimensional Embedding Migration', () {
    ...
    test('Full lifecycle of cross-dimensional embedding migration', () async {
      void logStep(String msg) => File('debug_log.txt').writeAsStringSync('$msg\n', mode: FileMode.append);
      logStep('--- TEST START ---');
      ...
      expect(backupCount, 5, reason: 'Migration backup should capture all 5 existing chunks');
      ...
      expect(stats['total_count'], 0, reason: 'Actual embeddings table should be empty after clearAndReinitEmbeddings');
      ...
      expect(verification.$1, isTrue, reason: 'allMigrated flag should be true');
      expect(verification.$2, isTrue, reason: 'noStaleData flag should be true');
      ...
      final searchResults = await db.searchSimilar(
        queryEmbedding: List.filled(5, 0.8), // Exact match simulation for chunk 0
        topK: 1,
        dimension: 5,
      );
      logStep('--- SEARCH DONE ---');
    ...
    test('Migration verification mechanism catches partial or stale state', () async {
      // Setup old data
      await seedEmbeddingData(db, count: 2, modelId: 'old_model', dimension: 3);
      ...
      expect(initialVerif.$2, isFalse, reason: 'Stale data still exists (old_model)');
      ...
      await db.markBackupChunkMigrated(unmigrated[0]['embedding_id'] as String);
      ...
      expect(partialVerif.$2, isTrue, reason: 'No stale data (old embeddings were cleared)');
      ...
      await db.markBackupChunkMigrated(unmigrated[1]['embedding_id'] as String);
      ...
      expect(staleVerif.$2, isFalse, reason: 'One chunk has wrong_model_xyz instead of new_model');
    });
  });
}
