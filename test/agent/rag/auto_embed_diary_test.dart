...
void main() {
  group('Auto-Embed and Supplement Logic Tests', () {
    ...
    setUp(() {
      ...
      when(() => mockApiConfig.setGlobalEmbeddingDimension(any())).thenAnswer((_) async {});
    ...
    test('2. getEmbeddedSourceMetadataByType 能够正确识别已嵌入的日记（补充嵌入逻辑测试）', () async {
      ...
      // 模拟先嵌入两篇日记
      await embeddingService.embedText(text: '日记1内容', sourceType: 'diary', sourceId: '1', groupId: 'diary_batch');
      await embeddingService.embedText(text: '日记2内容', sourceType: 'diary', sourceId: '2', groupId: 'diary_batch');
      // 模拟一个非日记的嵌入
      await embeddingService.embedText(text: '普通记忆', sourceType: 'chat', sourceId: 'mem_1', groupId: 'mem');
    ...
    test('3. reEmbedText 会删除原有数据并重新生成（长文分块测试）', () async {
      ...
      // 初始：写入一个短日记
      await embeddingService.embedText(text: '旧的短日记', sourceType: 'diary', sourceId: '999', groupId: 'diary_auto');
      ...
      final allDiary999Chunks = chunks.where((c) => c['source_id'] == '999').toList();
      expect(allDiary999Chunks.isNotEmpty, isTrue);
      expect(allDiary999Chunks.length, greaterThan(3), reason: '2000字被按 512 大小分出了多个 chunk');

      // 验证分块逻辑的正确性，第一个 chunk 应该具有 chunk_index 0
      final firstChunk = allDiary999Chunks.firstWhere((c) => c['chunk_index'] == 0);
      expect(firstChunk['chunk_text'].length, 512, reason: '由于中文字符不限制，chunk 长度应该是 512');
    });
  });
}
